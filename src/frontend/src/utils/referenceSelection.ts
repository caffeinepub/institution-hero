import { REFERENCES, type ReferenceEntry } from "../content/references";
 ...
 */
export function selectValidReferenceByHash(
  inputText: string,
  ...
  if (candidateKeys && candidateKeys.length > 0) {
    const validCandidates = candidateKeys
      ...
      .filter(
        (ref): ref is ReferenceEntry =>
          ref !== undefined && isValidReference(ref),
      );
